...
export default function VenteClient({ boutique, produits, clients, ventes }: {
  ...
  const {
    lire, parle, ecoute,
    commencerEcoute, arreterEcoute,
    annoncerTotal, confirmerVente, alerteStock,
    supportTTS, supportSTT,
  } = useVocal({ langue: 'fr-FR' })
  ...
  function addToCart(produit: Produit) {
    ...
    // Annonce vocale si actif
    if (vocalActif) lire(`${produit.nom} ajouté`)
    // Alerte stock bas
    if (vocalActif && produit.stock <= (boutique.stock_alerte || 5)) {
      alerteStock(produit.nom)
    }
    setSearch('')
  }
  ...
  // ── Commande vocale produit ───────────────────────────────────────────────
  function lancerRechercheVocale() {
    if (!supportSTT) { alert('Reconnaissance vocale non disponible'); return }
    commencerEcoute({
      onResult: (texte) => {
        // Chercher produit correspondant
        const texteNorm = texte.toLowerCase()
        const produit = produits.find(p =>
          texteNorm.includes(p.nom.toLowerCase()) ||
          p.nom.toLowerCase().includes(texteNorm)
        )
        if (produit) {
          addToCart(produit)
          lire(`${produit.nom} ajouté au panier`)
        } else {
          setSearch(texte)
          lire(`Recherche : ${texte}`)
        }
      },
      onErreur: (err) => lire('Micro non disponible'),
    })
  }
  ...
  async function handleVente() {
    ...
    if (!montantPaye) {
      if (vocalActif) lire('Veuillez entrer le montant reçu')
      else alert('Entrez le montant recu')
      return
    }
    if (modePaiement === 'especes' && paye < total) {
      if (vocalActif) lire('Montant insuffisant')
      else alert('Montant insuffisant')
      return
    }
    setLoading(true)
    ...
    for (const item of cart) {
      const { error: rpcError } = await supabase.rpc('decrement_stock', { p_id: item.produit_id, qty: item.quantite })
    ...
    const monnaie = Math.max(0, monnaieRendue)

    // Confirmation vocale
    if (vocalActif) {
      confirmerVente(total, monnaie, devise)
    }
          ...
          {rendu > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-xl px-6 py-4 mb-4 inline-block">
              <p className="text-green-700 font-bold text-xl">
                Monnaie a rendre : {rendu.toLocaleString()} {devise}
              </p>
            </div>
          )}
          {parle && (
            <p className="text-xs text-gray-400 mb-3">🔊 Lecture en cours…</p>
          )}
          <div className="flex gap-3 justify-center mt-6">
            <button onClick={() => setSuccess(false)} className="btn-primary px-6">
              Nouvelle vente
            </button>
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        {supportTTS && (
          <button
            onClick={() => {
              setVocalActif(!vocalActif)
              if (!vocalActif) lire('Mode vocal activé')
            }}
            className={
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={'flex-1 py-2 rounded-xl text-sm font-medium ' + (tab === 'vente' ? 'bg-brand-500 text-white' : 'bg-white border text-gray-600')}
        >
          🛒 Vente
        </button>
        <button
          onClick={() => setTab('historique')}
          className={'flex-1 py-2 rounded-xl text-sm font-medium ' + (tab === 'historique' ? 'bg-brand-500 text-white' : 'bg-white border text-gray-600')}
        >
          📋 Historique
        </button>
      </div>
          ...
          <div className="card">
            <div className="flex gap-2 mb-3">
              <input
                ...
                placeholder="🔍 Rechercher produit ou code barre..."
                value={search}
                onChange={e => setSearch(e.target.value)}
              />
              {/* Commande vocale produit */}
              {supportSTT && vocalActif && (
                <button
                  onClick={ecoute ? arreterEcoute : lancerRechercheVocale}
                  className={
                    'px-3 rounded-xl text-sm font-medium transition ' +
                    (ecoute ? 'bg-red-500 text-white animate-pulse' : 'bg-brand-100 text-brand-600 hover:bg-brand-200')
                  }
                  title="Dire un produit à voix haute"
                >
                  {ecoute ? '⏹' : '🎤'}
                </button>
              )}
            ...
            {ecoute && (
              <div className="bg-red-50 border border-red-200 rounded-xl px-3 py-2 mb-2 text-center">
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        ...
                        {p.stock <= 5 && <span className="text-orange-500 ml-1">⚠️ bas</span>}
                      </p>
                    </div>
                    <span className="text-brand-600 font-bold text-sm">
                      {p.prix_vente.toLocaleString()} {devise}
                    </span>
                  </button>
                ))}
                {filteredProduits.length === 0 && (
                  <p className="text-gray-400 text-sm text-center py-2">Aucun produit trouve</p>
                )}
              </div>
            )}
          </div>
          ...
          {cart.length > 0 && (
            <div className="card">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-gray-700">🛒 Panier</h3>
                {/* Lire le total à voix haute */}
                {vocalActif && (
                  <button
                    onClick={() => annoncerTotal(total, devise)}
                ...
                {cart.map(item => (
                  <div key={item.produit_id} className="flex items-center justify-between py-2 border-b border-gray-100">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400">{item.prix_unitaire.toLocaleString()} × {item.quantite}</p>
                    </div>
              ...
              <div className="bg-brand-50 rounded-xl px-4 py-3 mb-4">
                <div className="flex justify-between font-bold text-brand-800">
                  <span>TOTAL</span>
                  <span>{total.toLocaleString()} {devise}</span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                    ...
                    <select className="input" value={clientId} onChange={e => setClientId(e.target.value)}>
                      ...
                      {clients.map(c => (
                        <option key={c.id} value={c.id}>
                          {c.nom} {c.dette > 0 ? '(dette: ' + c.dette.toLocaleString() + ')' : ''}
                        </option>
                      ))}
                    </select>
                  </div>
                ...
                {paye >= total && paye > 0 && (
                  <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-center">
                    <p className="text-green-700 font-bold">
                      Monnaie a rendre : {Math.max(0, monnaieRendue).toLocaleString()} {devise}
                    </p>
                  </div>
                )}
                ...
                >
                  {loading ? 'Enregistrement...' : 'Valider la vente — ' + total.toLocaleString() + ' ' + devise}
                </button>
              </div>
            </div>
          )}
            ...
            ventes.map((v: any) => (
              <div key={v.id} className="card">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="font-semibold text-gray-800">{v.montant_total.toLocaleString()} {devise}</p>
                    <p className="text-xs text-gray-400">{new Date(v.created_at).toLocaleString('fr-FR')}</p>
                    <span className={
                      'text-xs px-2 py-0.5 rounded-full ' +
                      (v.statut === 'payee' ? 'bg-green-100 text-green-700' :
                       v.statut === 'credit' ? 'bg-red-100 text-red-700' :
                       v.statut === 'retournee' ? 'bg-gray-100 text-gray-500' :
                       'bg-yellow-100 text-yellow-700')
                    }>
                      {v.statut === 'payee' ? '✅ Payee' :
                       v.statut === 'credit' ? '📝 Credit' :
                       v.statut === 'retournee' ? '↩️ Retournee' : '⚠️ Partiel'}
                    </span>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-xs text-gray-400">{v.mode_paiement}</span>
                    {/* Bouton retour direct depuis historique */}
                    {v.statut !== 'retournee' && (
                      <Link
                        href={'/boutique/' + boutique.id + '/retours?vente=' + v.id}
